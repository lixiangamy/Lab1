...
public class operational_control {
	public static String cont(String str1,String str2)throws IOException
	{
		out_c out_c1=new out_c();
		...
		if(!str1.equalsIgnoreCase("end"))
		{
			if(str1.length()>=5&&str1.substring(0,5).equalsIgnoreCase("!d/d "))
			{
				...
				{
					out_c1.outp("错误：未输入合法多项式！");
				}
				else
				{
					str2=polynomial1.derivative(str2,str1.substring(5));
				}
			}
			else if(str1.length()>=10&&str1.substring(0,10).equalsIgnoreCase("!simplify "))
			{
				...
				{
					polynomial1.simplify(str2,str1.substring(10));
				}
			}
			else
			{
				str2=polynomial1.expression(str1,str2);//前缀性变量报错
			}
		}
		return str2;
	}
...
class in_c{
	public String inp()throws IOException
	{
		String str1;
		System.out.print(">");
		BufferedReader buf;
...
class out_c{
	public void outp(String result_out)
	{
...
class polynomial{
	...
	int count=0;
	String neirong="";
	public String deleempty(String str1)
	{
		int i;
	...
	public String addxing(String str1)
	{
		int i;
		for(i=0;i<str1.length();i++)
		{
	...
	public String easyre(String s_re)
	{
		int i=0,biao=0,num=0,add_num=0,sub_num=0,mn=0;
	...
	public String expression(String str1,String str2)
	{
			out_c out_c2=new out_c();
			String s2,z,sh,dan,res,cun,s_rnd,rn,va_n,re_n,st4="",s_re="",str3="";
			int i,j,h=0,add_num,r,a,sub_num,num,q,begin,kk=0,nn,p=0,m=0,ch=0,xis,xi_n;
			str1=deleempty(str1);
			if(str1.equals(""))
			{
				out_c2.outp("polynomial is illegal");
				tureornot=0;
				return str2;
			}
			for(i=0;i<str1.length();i++)
			...
			for(i=0;i<sh.length();i++)
			{
				if(sh.charAt(i)>='a'&&sh.charAt(i)<='z'||sh.charAt(i)>='A'&&sh.charAt(i)<='Z')
				{
					...
					if(cun.indexOf(z)!=-1)
					{
						out_c2.outp("variable is illegal");
						tureornot=0;
						return str2;
					}
					cun=cun+" "+z;
			...
			do
			{
				monomial monomial12=new monomial();
				add_num=s2.indexOf('+');
				sub_num=s2.indexOf('-');
				if(add_num<sub_num)
				{
				...
				ch=dan.length();
				dan=monomial12.d_xi(dan);
				dan=monomial12.d_hebing(dan);
	...
	public void simplify(String str2,String x)
	{
		int i,j,q,p,r,a,f=0,h=0,sit,add_num,m,sub_num,num,biao,fine=0;
		float xi,xi_n,nn;
		count=0;
		out_c out_c22=new out_c();
		monomial monomial33=new monomial();
		String x_n=x,rn,va,va_n,result,s_re="",dan="",s_ren,s_rnd,re_n;//s_re是结果的和，result是单项式结果
		for(j=0;j<x_n.length();j++)
		{
			if(x_n.charAt(j)=='=')
			{
				...
				if(f>0)//变量输入非法
				{
					out_c22.outp("variable is illegal");
				}
				sit=str2.indexOf(va);
				if(sit==-1)//没有变量
				{
					out_c22.outp("Error, no variable");
					return;
				}
	...
	public String derivative(String str2,String x)//str2为表达式，x为变量
	{
		int sit,i,f=0,add_num,sub_num,num,fine;
		out_c out_c3=new out_c();
		String dan,news2="",resu="";//dan为开始的单项式，news2为新多项式
		count=0;
		if(x.equalsIgnoreCase(""))
		{
			out_c3.outp("variable is null");
			return str2;
		}
		...
		if(f>0)//变量输入非法
		{
			out_c3.outp("variable is illegal");
			return str2;
		}
		sit=str2.indexOf(x);
		if(sit==-1)//没有变量
		{
			out_c3.outp("Error, no variable");
			return str2;
		}
		...
		do
		{
			...
			monomial1.mnei=dan;
			resu=monomial1.d_derivative(dan,x);
		if(count==1)
		{
			if(fine==1)
			{
...
class monomial{
	int ch;
	String mnei="";
	public String d_derivative(String dan,String x)
	{
		String resu="";
		int sit,i,mi,xishu;
		sit=dan.indexOf(x);
	...
	public String d_hebing(String dan)
	{
		int i=0,j=0,p=0,q=0,sit=0,pq=0,count=0;
		String dans="",dant="",var="";
		dans=dan;
	...
	public String d_sim(String dan,String x)
	{
		var var1=new var();
		String result=dan;
		String xnn=x;
		int j=0,k=0,i=0,p=0,q=0,mi=0,d_sit=0;
		float xi,xi_n;
		for(j=0;j<xnn.length();j++)
		{
			if(xnn.charAt(j)=='=')
			{
				var1.va=xnn.substring(0,j);//变量
				...
				var1.vn_st=xnn.substring(j+1,k);//变量值的字符串
				var1.vn=Float.parseFloat(xnn.substring(j+1,k));//变量的值
				d_sit=result.indexOf(var1.va);
				if(d_sit==-1)
				{			
				}
				else if(result.equals(var1.va))
				{
					result=var1.vn_st;//则取变量前的所有内容为导数
				}
				else if(result.substring(result.length()-var1.va.length()).equals(var1.va))//若变量在单项式末尾
				{
					if(result.charAt(0)>='0'&&result.charAt(0)<='9'||result.charAt(0)=='-')
					{
						for(i=0;i<result.length()&&(result.charAt(i)>='0'&&result.charAt(i)<='9'||result.charAt(i)=='.'||result.charAt(i)=='-');i++);
						xi=Float.parseFloat(result.substring(0,i));//系数
						result=String.valueOf(xi*var1.vn)+result.substring(i,result.indexOf(var1.va)-1);//则取变量前的所有内容为导数
					}
					else
					{
						if(var1.vn==1)
						result=result.substring(0,result.indexOf(var1.va)-1);
						else
						result=var1.vn_st+'*'+result.substring(0,result.indexOf(var1.va)-1);
					}
				}
				else if(!result.substring(result.length()-var1.va.length()).equals(var1.va))
				{
					if(result.charAt(result.indexOf(var1.va)+var1.va.length())!='^')//没有幂
					{
						if(result.indexOf(var1.va)==0)//若x在单项式开头
						{
							if(var1.vn==1)
							{
								result=result.substring(var1.vn_st.length()+1);
							}
							else
							{
								result=var1.vn_st+result.substring(var1.vn_st.length());
							}
						...
						else
						{
							if(var1.vn==1)
							{
								result=result.substring(0, result.indexOf(var1.va))+result.substring(result.indexOf(var1.va)+var1.va.length()+1);
							}
							else
							{
								if(result.charAt(0)>='0'&&result.charAt(0)<='9'||result.charAt(0)=='-')
								{
									...
									xi=Float.parseFloat(result.substring(0,i));//系数
									result=String.valueOf(xi*var1.vn)+result.substring(i,result.indexOf(var1.va))+result.substring(result.indexOf(var1.va)+var1.va.length()+1);//则取变量前的所有内容为导数
								}
								else
								{
									result=var1.vn_st+'*'+result.substring(0,result.indexOf(var1.va))+result.substring(result.indexOf(var1.va)+var1.va.length()+1);
								}
							}
						}
					}//end of 没有幂
					else//有幂
					{
						...
						{
							for(i=result.indexOf(var1.va)+var1.va.length()+1;i<result.length()&&result.charAt(i)!='*';i++);
							mi=Integer.parseInt(result.substring(result.indexOf(var1.va)+var1.va.length()+1,i));//幂次
							...
							result=String.valueOf(xi)+result.substring(i,result.indexOf(var1.va)-1)+result.substring(result.indexOf(var1.va)+var1.va.length()+1+String.valueOf(mi).length());
						}
						else//开头没系数
						{
							for(i=result.indexOf(var1.va)+var1.va.length()+1;i<result.length()&&result.charAt(i)!='*';i++);
							...
							if(result.indexOf(var1.va)!=0)
							result=String.valueOf(xi)+'*'+result.substring(0,result.indexOf(var1.va)-1)+result.substring(result.indexOf(var1.va)+var1.va.length()+1+String.valueOf(mi).length());
							else
							result=String.valueOf(xi)+result.substring(result.indexOf(var1.va)+var1.va.length()+1+String.valueOf(mi).length());
						}	
					}//end of 有幂
				}//end of 不在结尾
